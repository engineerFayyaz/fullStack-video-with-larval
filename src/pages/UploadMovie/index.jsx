...
const UploadMoviePage = () => {
  ...
  const handleFileChange = (fieldName, file) => {
    // Append the file to the existing formData
  ...
  const handlePosterChange = (e) => {
    const file = e.target.files[0];
    handleFileChange("poster_image", file);
  };

  const handleProofChange = (e) => {
    const file = e.target.files[0];
    handleFileChange("proof", file);
  };
  ...
  const handleSubmit = () => {
    ...
    if (formData.proof_ownership) {
      formDataToSend.append("proof_ownership", formData.proof);
    }
